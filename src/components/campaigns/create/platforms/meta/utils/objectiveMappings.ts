...
/**
 * Call to action types for different objectives
 */
export const CALL_TO_ACTION_TYPES = {
    LEAD_GENERATION: ['SIGN_UP', 'LEARN_MORE', 'APPLY_NOW', 'GET_QUOTE', 'SUBSCRIBE'],
    TRAFFIC: ['LEARN_MORE', 'SHOP_NOW', 'VIEW_MORE', 'VISIT_WEBSITE', 'CONTACT_US'],
    POST_ENGAGEMENT: ['LIKE_PAGE', 'LEARN_MORE', 'VIEW_INSTAGRAM_PROFILE'],
    APP_PROMOTION: ['INSTALL_MOBILE_APP', 'PLAY_GAME', 'USE_APP', 'OPEN_APP'],
    CONVERSIONS: ['SHOP_NOW', 'BUY_NOW', 'BOOK_TRAVEL', 'GET_OFFER']
};
...
export function getDefaultOptimizationGoal(objective: string, destinationType?: string): string {
    // If we have both objective and destinationType
    if (destinationType &&
        OBJECTIVE_DESTINATION_OPTIMIZATION_MAP[objective]?.[destinationType]) {
        return OBJECTIVE_DESTINATION_OPTIMIZATION_MAP[objective][destinationType].defaultGoal;
    }

    // If we only have objective, find the first destination type and use its default
    if (OBJECTIVE_DESTINATION_OPTIMIZATION_MAP[objective]) {
        const firstDestination = Object.keys(OBJECTIVE_DESTINATION_OPTIMIZATION_MAP[objective])[0]; // This would be forms
        if (firstDestination) {
            return OBJECTIVE_DESTINATION_OPTIMIZATION_MAP[objective][firstDestination].defaultGoal;
        }
    }

    // Fallback defaults based on objective
    switch (objective) {
        case CAMPAIGN_OBJECTIVES.LEADS:
            return OPTIMIZATION_GOALS.LEAD_GENERATION;

        default:
            return OPTIMIZATION_GOALS.LEAD_GENERATION; // Safe default
    }
}
...
export function buildPromotedObject(
    ...
    requiredFields.forEach(field => {
        if (params[field as keyof typeof params]) {
            promotedObject[field] = params[field as keyof typeof params];
        }
    });
