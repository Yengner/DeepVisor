...
export default function ConnectAccountsStep({ onNext, onPrev, userData }: ConnectAccountsStepProps) {
  ...
  const handleConnect = async (platform: string) => {
    ...
    try {
      ...
      window.location.href = `/api/integrations/connect/${platform}?returnTo=${returnTo}`;
    } catch (error) {
    ...
    <Stack gap="xl" py={16}>
      ...
      <Paper withBorder p="md" radius="md" className="max-w-xl mx-auto w-full">
        ...
        <Alert mt="md" icon={<IconInfoCircle size={16} />} color="blue" variant="light">
          Additional platforms can be connected later from your integration settings.
        </Alert>
      </Paper>
